...
class CouncilSystem:
    ...
    async def _agent_vote(self, agent: Agent, task: Dict) -> Vote:
        ...
        # For now, simulate realistic voting
        import random
    ...
    def _synthesize_decision(
        ...
        return CouncilDecision(
            ...
            conditions=self._generate_conditions(votes) if recommendation == "conditional" else []
        )
    
    def _generate_conditions(self, votes: List[Vote]) -> List[str]:
        """Generate conditions when decision is not unanimous"""
        # Extract concerns from reject/low-confidence votes
        conditions = []
        
        for vote in votes:
            if vote.vote == VoteType.REJECT or vote.confidence < 0.7:
                # In real implementation, extract specific concerns
    ...
    def get_council_stats(self) -> Dict[str, Any]:
        ...
        approvals = sum(1 for d in self.decision_history if d["decision"]["recommendation"] == "approve")
